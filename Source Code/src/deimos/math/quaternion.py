...
def quaternion_error(q, q_target):
    """q_e = q_target^-1 (x) q, scalar-first, both plain (4,) arrays.

    Array-based twin of Quaternion.error_quaternion() -- same convention,
    used by every controller (control/pd.py, wie.py, pid.py) since the
    propagator hands them raw arrays, not Quaternion objects.
    """
    w_t, x_t, y_t, z_t = q_target
    qt_inv = np.array([w_t, -x_t, -y_t, -z_t])
    w1, x1, y1, z1 = qt_inv
    w2, x2, y2, z2 = q
