...
- Motor torque saturates at +/- max_torque; wheel speed saturates at
  +/- max_speed (control authority along a pinned wheel is lost).
...
class ReactionWheelArray:
    ...
    def __init__(
        self,
        axes=None,
        wheel_inertia=5.8e-6,    # [kg m^2] CAD-exported flywheel inertia (Al 7075 hub + rim)
        max_torque=4.0e-3,      # [N m]   software torque cap (Maxon ECX Flat 22L can do ~28.8e-3)
        max_speed=1256.6,        # [rad/s] 12000 rpm, per ECX Flat 22L datasheet
        tilt_deg=32.0,          # pyramid tilt angle beta from the body x-y plane
        config="orthogonal",       # "pyramid" (4 wheels) or "orthogonal" (3 wheels)
    ...
    def allocate(self, tau_cmd):
        ...
        for i in range(len(g)):
            if abs(g[i]) > self.max_torque:
                torque_saturated.append(True)
            else:
                torque_saturated.append(False)
